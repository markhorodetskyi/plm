...
if(ping<=800):
    ...
    dbOut = pymysql.connect(host="213.174.22.133", user="root", passwd="02071228", db="plm", port=3307)
    ...
    for i in range(2, len(avTabl)):
        ...
        curOut.execute("SELECT MAX(meterDate) FROM %s WHERE gadget_HW_id like'1'" %(tabToIns[2:-3]))
        ...
        if(maxId[0]==None):
            ...
            for a in select:
                for c in a[1:]:
                    dataToIns +=str(c)+','
                command = 'INSERT INTO plm.'+str(tabToIns[2:-3])+'('+str(colToIns)+') VALUES ('+dataToIns[:-1]+');'
                curOut.execute(command)
                dbOut.commit()
                counter+=1
                dataToIns = ''
